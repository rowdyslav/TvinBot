...
class Commands(commands.Cog):
    ...
    async def register(self, ctx: nextcord.Interaction, username: str, password: str):
        ...
        with open("config.json", "r") as f:
            config = json.load(f)
        account_limit = config.get("account_limit", {}).get(discord_id, 1)

        async with self.db_conn.acquire() as cursor:
            await cursor.execute(
                "SELECT COUNT(*) FROM users WHERE discord_id = $1", discord_id
            )
            ...
            if is_registered:
                await ctx.response.send_message("Никнейм уже занят!")
                return
        ...
        await ctx.response.send_message("Вы были успешно зарегистрированы!")
